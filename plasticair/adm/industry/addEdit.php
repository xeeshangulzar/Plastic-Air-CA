...
$industryID=(!empty($_REQUEST["industryID"]))?addslashes($_REQUEST["industryID"]):"";
...
$clearance=(!empty($_REQUEST["clearance"]))?str_replace("'", "", $_REQUEST["clearance"]):"";
	...
	{			
		...
		if(!empty($_FILES["File"]['name'])){
			...
			$pictpath=uplPictures($_FILES["File"],
									"/dat/industry/".$site_imagesID.".jpg",
									"900",
									"900",
									"4048000",
									"1",
									...
									"");
			$pictpath=uplPictures($_FILES["File"],
									"/dat/industry/".$site_imagesID."_s.jpg",
									"180",
									"180",
									"4048000",
									"1",
			...
			//print "PP->".$pictpath;
			if($pictpath!=""){
				$sSQL="UPDATE site_images SET filePath='/dat/industry/".$site_imagesID.".jpg',
								filePath_s='/dat/industry/".$site_imagesID."_s.jpg'
...
<tr valign="top"><td align="right">Headline: </td>
<td><textarea type="text" name="headline" class="inputField" style="width:400px; height:70px;"><?=$headline?></textarea></td></tr>
...
<td align="left" colspan="3" valign="top"><input type="file" name="File" id="File" /></td></tr><? 
    ...
    while($row=mysql_fetch_assoc($result)) {?><div style="float:left; border:#999 1px solid; margin:5px;">
		<img src="<?=$row['filePath_s']?>?ttt=<?=mktime()?>" /><br />
