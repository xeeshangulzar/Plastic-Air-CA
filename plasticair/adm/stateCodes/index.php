...
$numPerPage=(!empty($_REQUEST["numPerPage"]))?str_replace("'","",$_REQUEST["numPerPage"]):"20";
$sort=(!empty($_REQUEST["sort"]))?str_replace("'","",$_REQUEST["sort"]):"countryCode, stateName";
$sortdir=(!empty($_REQUEST["sortdir"]))?addslashes($_REQUEST["sortdir"]):"1";
...
if(!empty($chkItem) && $listAction=="1" && $delItem=="1")
{
	...
	$sSQL="SELECT *
			FROM ".$tableName."
			WHERE ".$tableID." IN (".addslashes($chkItemString).")";
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$sSQL="SELECT *
			FROM files
			WHERE tableIDNum = '".$row[$tableID]."' AND tableName = '".$tableName."'";
		...
		while($row33 = mysql_fetch_array($result33))
		{
			...
			$sSQL2="DELETE FROM files
						WHERE filesID = '".$row33['filesID']."'";
...
{
	$sSQLwhere.=" AND ( title LIKE '%".$keyWord."%' OR headline LIKE '%".$keyWord."%' OR description LIKE '%".$keyWord."%')";	
...
<?php /*?><tr><td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="optionsTable">
...
<td><input type="image" src="/adm/img/btn_go.png" width="23" height="20" /></td><br />
<td width="100%"></td></tr>
...
</script></table></td></tr><?php */?>
                    <tr>
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="pagingTable" style="height:32px;">
                      
                        <tr>
                          <td align="left">Listings <?=($fromRecord+1)?>-<?=(($fromRecord+$numPerPage)<=$numrows)?$fromRecord+$numPerPage:$numrows?> of <?=$numrows?>  | Show 
...
<td width="8"></td>
<td width="19" class="orangeCheckbox" align="center"><a href="#" onClick="javascript: setCheckboxes('ff1', !(allChecked)); return false;"><img src="/adm/img/listingTitles_checkbox.jpg" width="21" height="22" border="0"></a></td>
<td width="10"></td>
<td><A href="#" onClick="javascript: sort_list('stateName'); return false;" class="listingHeaderLinks">Province</a></td>
<td><A href="#" onClick="javascript: sort_list('countryCode'); return false;" class="listingHeaderLinks">Country</a></td>
<td><A href="#" onClick="javascript: sort_list('date'); return false;" class="listingHeaderLinks">Date</a></td>
<td><A href="#" onClick="javascript: sort_list('gst'); return false;" class="listingHeaderLinks">GST</a></td>
<td><A href="#" onClick="javascript: sort_list('pst'); return false;" class="listingHeaderLinks">PST</a></td>
<td><A href="#" onClick="javascript: sort_list('hst'); return false;" class="listingHeaderLinks">HST</a></td>
<td><A href="#" onClick="javascript: sort_list('tax'); return false;" class="listingHeaderLinks">TAX</a></td></tr><?
$result=mysql_query($sSQL) or die ("MySQL err: ".mysql_error()."<br>".$sSQL);
$bgColor="FFFFFF";
while($row=mysql_fetch_array($result)) {
		?><tr onMouseOver='javascript: act_row(this);' onmouseout="javascript: inact_row(this,'<?=$bgColor?>')"  onClick="window.location='addEdit.php?<?=$tableID?>=<?=$row[$tableID]?>'; return false;" class="arial12_black" bgcolor="<?=$bgColor?>">
		...
		<td style="padding:3px;"><?=$row['stateName']?></td>
		<td style="padding:3px;"><?=$row['countryCode']?></td> 
		<td style="padding:3px;"><?=$row['date']?></td>
		<td style="padding:3px;"><?=$row['gst']?></td>
		<td style="padding:3px;"><?=$row['pst']?></td>
		<td style="padding:3px;"><?=$row['hst']?></td>
		<td style="padding:3px;"><?=$row['tax']?></td></tr><?
...
<option value="1">Delete</option></select><img src="/adm/img/btn_go.png" width="23" height="20" align="absmiddle" onclick="listActionRun()" style="cursor:pointer"></td></tr></table></td></tr></table></td></tr></table></td></tr></table></form>
