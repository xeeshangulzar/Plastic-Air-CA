...
//		$sSQL2="DELETE FROM products_dimensions WHERE ".$tableID." IN (".addslashes($chkItemString).")";
//		mysql_query($sSQL2) or die ("MySQL err: ".mysql_error()."<br>".$sSQL2);	
//		$sSQL2="DELETE FROM products_features WHERE ".$tableID." IN (".addslashes($chkItemString).")";
//		mysql_query($sSQL2) or die ("MySQL err: ".mysql_error()."<br>".$sSQL2);	
...
<td><select name="product_categoryID" id="product_categoryID" class="inputField" style="width:200px;"><option value="">Select Category</option><?
...
<td><A href="#" onClick="javascript: sort_list('product_categoryID'); return false;" class="listingHeaderLinks">Category</a></td>
...
<td style="padding:3px;"><?=$arr_product_category[$row['product_categoryID']]?><? /*if(!empty($row['product_categoryID'])){$arr_c=explode(",", $row['product_categoryID']);
	foreach($arr_c as $v){ ?><?=$arr_product_category[$v]?>; <? }}*/?></td>
