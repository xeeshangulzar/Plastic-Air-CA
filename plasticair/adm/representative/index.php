...
$sSQL="SELECT * FROM stateCodes ";
$result1=mysql_query($sSQL) or die ("MySQL err: ".mysql_error()."<br>".$sSQL);
$arr_stateCodes=array();
while($row1=mysql_fetch_array($result1)){
		$arr_stateCodes[$row1['stateCode']]=$row1['stateName'];}
//paging
...
if(!empty($_REQUEST['searchFromSession'])){
	$countryCode=(!empty($_SESSION[$tableName."_countryCode"]))?$_SESSION[$tableName."_countryCode"]:$countryCode;
	...
	$_SESSION[$tableName."_countryCode"]=$countryCode;
...
if(!empty($countryCode)){$sSQLwhere.=" AND FIND_IN_SET('$countryCode', countryCode) ";}
...
<td><select name="countryCode" id="countryCode" class="inputField" style="width:200px;"><option value="">Select Country</option><?
...
<td><A href="#" onClick="javascript: sort_list('countryCode'); return false;" class="listingHeaderLinks">Province</a></td>
...
<td style="padding:3px;"><?=empty($arr_countryCodes[$row['countryCode']])?'':$arr_countryCodes[$row['countryCode']]?></td>
<td style="padding:3px;"><?=empty($arr_stateCodes[$row['stateCode']])?'':$arr_stateCodes[$row['stateCode']]?></td></tr><?
