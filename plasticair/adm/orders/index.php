...
{
	$sSQLwhere.=" AND ( email LIKE '%".$keyWord."%' OR fname LIKE '%".$keyWord."%' OR lname LIKE '%".$keyWord."%' OR total LIKE '%".$keyWord."%' OR ordersID = '".$keyWord."')";	
                    ...
                    <tr>
                      ...
                      <td align="left"><table border="0" cellspacing="0" cellpadding="0" height="35">
                        <tr>
                          ...
                          <?php /*?><td background="/adm/img/tabs_bg.jpg"><a href="/adm/<?=$folderName?>/addEdit.php" class="tabLink">Add New</a></td>
                    ...
                    <tr>
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr class="listingTitles">
                          ...
                          <td width="10"></td>
                          <td><A href="#" onClick="javascript: sort_list('ordersID'); return false;" class="listingHeaderLinks">ID</a></td>
                          <td><A href="#" onClick="javascript: sort_list('dateCreated'); return false;" class="listingHeaderLinks">Date</a></td>
                          <td><A href="#" onClick="javascript: sort_list('status'); return false;" class="listingHeaderLinks">Status</a></td>
                          <td><A href="#" onClick="javascript: sort_list('fname,lname'); return false;" class="listingHeaderLinks">User</a></td>
                          <td><A href="#" onClick="javascript: sort_list('shippingAddress'); return false;" class="listingHeaderLinks">Shipping</a></td>
                          <td><A href="#" onClick="javascript: sort_list('billingAddress'); return false;" class="listingHeaderLinks">Billing</a></td>
                          <td><A href="#" onClick="javascript: sort_list('grandTotal'); return false;" class="listingHeaderLinks">Totals</a></td>
                        </tr>
						<?
						$result=mysql_query($sSQL) or die ("MySQL err: ".mysql_error()."<br>".$sSQL);
						$bgColor="FFFFFF";
                        ...
                        <tr onMouseOver='javascript: act_row(this);' onmouseout="javascript: inact_row(this,'<?=$bgColor?>')"  onClick="window.location='/adm/<?=$folderName?>/addEdit.php?<?=$tableID?>=<?=$row[$tableID]?>'; return false;" class="arial12_black" bgcolor="<?=$bgColor?>">
                          ...
                          <td style="padding:3px;"><?=$row['ordersID']?></td>
                          <td style="padding:3px;"><?=$row['dateCreated']?></td>
                          <td style="padding:3px;"><?=$row['status']?></td>
                          <td style="padding:3px;"><?=$row['fname']." ".$row['lname']?><br /><?=$row['email']?></td>
                          <td style="padding:3px;">
						  <?=$row['shippingAddress']?><br />
						  <?=$row['shippingCity']?>,<?=$row['shippingStateCode']?><br />
						  <?=$row['shippingCountry']?>,<?=$row['shippingZip']?><br />
						  <?=$row['shippingContact']?>,<?=$row['shippingPhone']?>
						  </td>
						  <td style="padding:3px;">
						  <?=$row['billingAddress']?><br />
						  <?=$row['billingCity']?>,<?=$row['billingStateCode']?><br />
						  <?=$row['billingCountry']?>,<?=$row['billingZip']?><br />
						  <?=$row['billingContact']?>,<?=$row['billingPhone']?>
						  </td>
						  <td style="padding:3px;">
						  Sub total: $<?=number_format($row['subTotal'],2)?><br />
						  <?php /*?>Taxes: $<?=number_format($row['taxAmount'],2)?> (<?=$row['taxRate']?>%)<?php */?><?
                          
					$taxStr="";
					if(!empty($row['gst'])){
						?>GST: $<?=number_format($row['tax_gst'], 2)."(".$row['gst']."%)"?><br /><?
						}
					if(!empty($row['pst'])){
						?>PST: $<?=number_format($row['tax_pst'], 2)."(".$row['pst']."%)"?><br /><?
						}
					if(!empty($row['hst'])){
						?>HST: $<?=number_format($row['tax_hst'], 2)."(".$row['hst']."%)"?><br /><?
						}
					if(!empty($row['tax'])){
							?>TAX: $<?=number_format($row['tax_tax'], 2)."(".$row['tax']."%)"?><br /><?
						}
						  ?>Total: $<?=number_format($row['total'],2)?></td></tr><?
                    ...
                    <tr> 
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="listingBottom">
                        <tr>
                          ...
                          <img src="/adm/img/btn_go.png" width="23" height="20" align="absmiddle" onclick="listActionRun()" style="cursor:pointer"></td>
                        </tr>
                      </table></td>
                    </tr>
                  </table></td>
